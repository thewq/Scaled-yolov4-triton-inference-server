...
if __name__ == '__main__':
    ...
    if FLAGS.mode == 'image':
        ...
        if FLAGS.shm:
            ...
            output_buffer = results.get_output("prob")
            ...
            result = results.as_numpy('prob')[0]
            #print(result[0,0,0])
            print(f"Received result buffer of size {result.shape}")
            print(f"Naive buffer sum: {np.sum(result)}")
        ...
        for box in detected_objects:
            ...
            input_image = render_box(input_image, box.box(), color=tuple(RAND_COLORS[int(box.classID % 64)].tolist()))
