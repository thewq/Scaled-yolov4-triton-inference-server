...
namespace nvinfer1
{
    ...
    MishPlugin::MishPlugin(const void* data, size_t length) noexcept
    {
    ...
    void MishPlugin::forwardGpu(const float *const * inputs, float* output, cudaStream_t stream, int batchSize) noexcept {
    ...
    int MishPlugin::enqueue(int batchSize, const void*const * inputs, void** outputs, void* workspace, cudaStream_t stream) noexcept
    {
        //assert(batchSize == 1);
        //GPU
